<template>
  <div>
    <div v-for="(item, index) in list" :key="index">
      ...
      <div v-for="(itemx, index1) in item.areaList" :key="index1">
        <div @click="getVideo(itemx,index1,$event)">
          {{ itemx.areaName }}
          <van-list v-show="videoListFlag">
      ...
      <div>
        ...
        <van-list v-if="item.areaList.length == 0">
          <van-cell v-for="(itemx, indexl) in skyVideoList" :key="indexl" :title="itemx.municipalName" @click.native="go(itemx)">
...
export default {
  ...
  methods: {
    go(itemxl) {
      window.location.href = itemxl.municipalUrl
    ...
    async getVideo(item, index, $event) {
      console.log($event)
